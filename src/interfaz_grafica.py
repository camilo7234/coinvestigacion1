...
class Aplicacion(tk.Tk):
    ...
    def build_curve_tab(self, parent):
        ...
        controls = ttk.Frame(f, padding=10)
        controls.pack(fill="x")
        ttk.Label(controls, text="Seleccionar Muestra:").pack(side="left", padx=5)
        self.cmb_curve = ttk.Combobox(controls, state="readonly", width=30)
    ...
    def build_iot_tab(self, parent):
        ...
        tk.Label(f, text="Servidor de Comunicaciones Activo", font=("Arial", 14, "bold"), bg=COLOR_BG, fg=COLOR_FG, pady=20).pack()
    ...
    def update_detail_view(self):
        ...
        if self.current_data is not None:
            for _, m in self.current_data.iterrows():
                ...
                self.tree_detail.insert("", "end", values=(f"MUESTRA: {m.get('title')}", f"{nivel:.2f}", "%", estado_texto), tags=(canon,))

    def update_curve_selector(self):
        if self.current_data is not None:
            titles = [f"{i}: {row['title']}" for i, row in self.current_data.iterrows()]
            self.cmb_curve["values"] = titles
            if titles: self.cmb_curve.current(0)

    def query_sessions(self):
        try:
            start, end = self.date_start.get_date(), self.date_end.get_date()
            dev, sid_text = self.device_cmb.get(), self.id_entry.get().strip()
            sql = """SELECT s.id, s.filename, s.loaded_at::date, m.device_serial, m.classification_group, m.contamination_level 
                     FROM sessions s JOIN measurements m ON s.id = m.session_id WHERE s.loaded_at::date BETWEEN %s AND %s"""
            params = [start, end]
            if sid_text:
                try: params.append(int(sid_text)); sql += " AND s.id = %s"
                except: pass
            if dev and dev != "TODOS": sql += " AND m.device_serial = %s"; params.append(dev)
            sql += " ORDER BY s.id DESC"
            conn = pg8000.connect(**DB_CONFIG)
            cur = conn.cursor(); cur.execute(sql, tuple(params)); rows = cur.fetchall(); conn.close()
        ...
        except Exception as e: log.error(f"Error en consulta: {e}")

    def load_devices(self):
        try:
            conn = pg8000.connect(**DB_CONFIG)
            cur = conn.cursor(); cur.execute("SELECT DISTINCT device_serial FROM measurements WHERE device_serial IS NOT NULL")
            devs = [r[0] for r in cur.fetchall()]; conn.close()
    ...
    def on_session_select(self, _=None):
        ...
        try:
            ...
            m_rows = cur.fetchall(); m_list = []
            for r in m_rows:
                label = label_from_group(r[3])
                m_list.append({"title": r[0], "device_serial": r[1], "curve_count": r[2], "classification_group": r[3], "clasificacion": label, "contamination_level": r[4], "pca_scores": r[5]})
            self.current_data = pd.DataFrame(m_list); conn.close()
            self.update_detail_view(); self.update_curve_selector(); self.show_curve(); self.show_pca()
        except Exception as e: log.error(f"Error cargando sesión: {e}")

    def show_curve(self):
        if self.current_data is None or self.cmb_curve.get() == "": return
        try:
            idx = self.cmb_curve.current(); data = self.current_data.iloc[idx]; scores = data.get("pca_scores")
            if not scores: return
            self.ax_curve.clear(); self.ax_curve.set_facecolor("#1e272e")
            self.ax_curve.plot(scores, color="#3498db", linewidth=1.5)
            self.ax_curve.set_title(f"Voltagrama: {data['title']}", color="white", fontsize=12)
            self.ax_curve.set_xlabel("Puntos", color="white"); self.ax_curve.set_ylabel("Corriente (A)", color="white")
            self.ax_curve.tick_params(colors="white"); self.ax_curve.grid(True, color="#34495e"); self.canvas_curve.draw()
        except: pass
    ...
    def show_pca(self):
        ...
        try:
            ...
            if len(self.current_data) > 1:
                levels = self.current_data["contamination_level"].tolist()
                self.ax_pca.bar(range(len(levels)), levels, color="#e74c3c")
                self.ax_pca.set_title("Nivel de Contaminación por Muestra", color="white")
            else: self.ax_pca.text(0.5, 0.5, "Datos insuficientes para análisis PCA", color="white", ha='center')
